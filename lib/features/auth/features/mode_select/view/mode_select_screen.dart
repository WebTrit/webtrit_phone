  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<ModeSelectCubit, ModeSelectState>(
      listener: (context, state) {
        ...
        if (state.status == ModeSelectStatus.ok) {
          context.router.push(SupportedLoginsScreenPageRoute(
            coreUrl: state.url,
            demo: state.demo,
            defaultTenantId: EnvironmentConfig.DEFAULT_TENANT_ID,
            supportedLogins: state.supportedLogin,
          ));
        }
      },
            ...
            actions: state.demo
                ? [
                    IconButton(
                      ...
                      onPressed: () => context.router.push(const LoginCoreUrlAssignScreenPageRoute()),
                    ),
                  ]
                : null,
          ),
