...
@DriftAccessor(
    tables: [SystemNotificationsTable, SystemNotificationsOutboxTable])
class SystemNotificationsDao extends DatabaseAccessor<AppDatabase>
    with _$SystemNotificationsDaoMixin {
  SystemNotificationsDao(super.db);

  //
  // Notifications section
  //
  Future<List<SystemNotificationData>> getNotifications(
    ...
    if (from != null)
      query.where((tbl) => tbl.createdAtRemoteUsec
          .isSmallerThanValue(from.microsecondsSinceEpoch));
    if (to != null)
      query.where((tbl) =>
          tbl.createdAtRemoteUsec.isBiggerThanValue(to.microsecondsSinceEpoch));
  ...
  Future<SystemNotificationData?> getNotificationById(int id) {
    return (select(systemNotificationsTable)..where((tbl) => tbl.id.equals(id)))
        .getSingleOrNull();
  }
      ...
      [
        leftOuterJoin(
          ...
          systemNotificationsOutboxTable.notificationId
                  .equalsExp(systemNotificationsTable.id) &
              systemNotificationsOutboxTable.actionType
                  .equals(SnOutboxDataActionType.seen.name),
        ),
      ],
    )
      ..addColumns([countAll()])
      ..where(systemNotificationsTable.seen.equals(false) &
          systemNotificationsOutboxTable.notificationId.isNull());
  ...
  Future<void> upsertNotifications(List<SystemNotificationData> notifications) {
    return batch((batch) => batch.insertAllOnConflictUpdate(
        systemNotificationsTable, notifications));
  }

  Future<int> deleteNotification(int id) {
    return (delete(systemNotificationsTable)..where((tbl) => tbl.id.equals(id)))
        .go();
  }
  ...
  Future<void> upsertOutboxNotification(
      SystemNotificationOutboxEntryData notification) {
    return into(systemNotificationsOutboxTable)
        .insertOnConflictUpdate(notification);
  }

  Future<int> deleteOutboxNotification(
      int id, SnOutboxDataActionType actionType) {
