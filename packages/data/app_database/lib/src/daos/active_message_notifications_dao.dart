...
class ActiveMessageNotificationsDao extends DatabaseAccessor<AppDatabase>
    with _$ActiveMessageNotificationsDaoMixin {
  ActiveMessageNotificationsDao(super.db);

  Future<List<ActiveMessageNotificationData>> getAllByConversation(
      int conversationId) {
    return (select(activeMessageNotificationsTable)
          ..where((t) => t.conversationId.equals(conversationId)))
        .get();
  }

  Future<int> set(ActiveMessageNotificationData notification) {
    return into(activeMessageNotificationsTable)
        .insertOnConflictUpdate(notification);
  }

  Future<int> deleteByNotification(String notificationId) {
    return (delete(activeMessageNotificationsTable)
