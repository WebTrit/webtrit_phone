...
class FavoritesDao extends DatabaseAccessor<AppDatabase> with _$FavoritesDaoMixin {
  ...
  Stream<List<FavoriteWithContactData>> watchFavoritesExt() {
    final q = (select(favoritesTable)..orderBy([(t) => OrderingTerm.asc(t.position)])).join([
      ...
      leftOuterJoin(presenceInfoTable, presenceInfoTable.number.equalsExp(_contactPhones.rawNumber)),
      leftOuterJoin(presenceInfoTable, presenceInfoTable.number.equalsExp(_contactPhones.sanitizedNumber)),
    ]);
  ...
  $ContactPhonesTableTable get _sourcePhone => alias(contactPhonesTable, 'source_phone');

  $ContactPhonesTableTable get _contactPhones => alias(contactPhonesTable, 'contact_phones');
}
