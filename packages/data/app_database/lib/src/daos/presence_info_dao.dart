...
class PresenceInfoDao extends DatabaseAccessor<AppDatabase>
    with _$PresenceInfoDaoMixin {
  PresenceInfoDao(super.db);

  Future<List<PresenceInfoData>> getPresenceInfoByNumber(String number) {
    final query = select(presenceInfoTable)
  ...
  Future<void> upsertPresenceInfo(List<PresenceInfoData> info) {
    return batch(
        (batch) => batch.insertAllOnConflictUpdate(presenceInfoTable, info));
  }

  Future<int> deletePresenceInfoByIds(List<String> ids) {
    final query = delete(presenceInfoTable)
      ..where((tbl) => tbl.idKey.isIn(ids));
    return query.go();
  }
}
