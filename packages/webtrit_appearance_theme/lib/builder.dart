...
class SchemaTablesBuilder implements Builder {
  ...
  Map<String, List<String>> get buildExtensions => const {
    r'$lib$': ['schema/enum_properties.g.dart', 'schema/union_variants.g.dart'],
  };
...
String enumPropertiesSource(Iterable<String> sources) {
  ...
  for (final info in parsed.classes) {
    ...
    // A class is described with its base's fields too, so the walk goes up
    // `extends` - within this package, which is as far as a base reaches here.
    for (var owner = info; ;) {
