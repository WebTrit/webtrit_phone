...
void main() {
  group('AppConfig.mainConfig parsing', () {
    ...
    test(
      'parses AppConfig and bottomMenu tabs with correct variants & values',
      () {
    ...
    test(
      'loginConfig [modeSelect screen] & settingsConfig basic fields parsed',
      () {
        final config = AppConfig.fromJson(json);
        ...
        final settingsSection = sections.firstWhere(
          (s) => s.titleL10n == 'settings_ListViewTileTitle_settings',
        );
  ...
  group('AppConfig.localization parsing', () {
    test(
      'the bundled config lists every supported language by default',
      () async {
        final json = await loadFixtureJson(
  ...
  group('RecentsTabScheme.supportsCallHistory parsing & useCdrs migration', () {
    ...
    test('new key is honoured', () {
      expect(
        recentsSupportsCallHistory({'supportsCallHistory': false}),
        isFalse,
      );
    ...
    test(
      'explicit null new key does not fall back to legacy useCdrs (default applies)',
      () {
        expect(
          recentsSupportsCallHistory({
            'supportsCallHistory': null,
            'useCdrs': false,
          }),
          isTrue,
        );
