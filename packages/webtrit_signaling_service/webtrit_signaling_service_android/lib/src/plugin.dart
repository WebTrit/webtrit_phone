...
class WebtritSignalingServiceAndroid extends SignalingServicePlatform {
  ...
  /// Events buffered since the last connect / reconnect.
  /// Replayed to late subscribers so they receive current session state.
  ...
  Future<void> _startHubMode(SignalingServiceConfig config, SignalingServiceMode mode) async {
    ...
    // Remove any stale hub port left in IsolateNameServer by a previous session.
    // Without this, _hubInitLoop would immediately find and subscribe to a dead
    // SendPort, then waste 500 ms per attempt on ack timeouts instead of waiting
    // for the new background isolate to register its fresh port.
    IsolateNameServer.removePortNameMapping(kSignalingHubPortName);
    _logger.fine('_startHubMode cleared stale hub port from IsolateNameServer');
