...
class AppleAppStoreClient extends BaseStoreClient {
  AppleAppStoreClient({super.httpClient});

  @override
  Future<StoreInfo?> getStoreInfo(String appPackageName) async {
    final lookupUrl = Uri.https('itunes.apple.com', '/lookup', {'bundleId': appPackageName});
    ...
    if (response.statusCode != 200) {
      ...
      try {
        ...
        if (resultsJson.isEmpty) {
          ...
          return StoreInfo(version: Version.parse(versionValue), viewUrl: Uri.parse(trackViewUrlValue));
        }
      } catch (e) {
        throw StoreInfoExtractorResponseFormatException(sourceError: e);
      }
    }
  }
}
