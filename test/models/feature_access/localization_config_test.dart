...
void main() {
  ...
  List<String> codes(List<Locale> locales) =>
  ...
  group('LocalizationConfig.resolve', () {
    ...
    test('filters to the requested subset, preserving bundle order', () {
      expect(codes(LocalizationConfig.resolve(bundled, const ['it', 'en'])), [
        'en',
        'it',
      ]);
    });
    ...
    test('unknown codes are ignored, valid ones kept', () {
      expect(
        codes(LocalizationConfig.resolve(bundled, const ['en', 'fr', 'de'])),
        ['en'],
      );
    });
    ...
    test('codes are matched case-insensitively and trimmed', () {
      expect(codes(LocalizationConfig.resolve(bundled, const [' EN ', 'IT'])), [
        'en',
        'it',
      ]);
    });
    ...
    test(
      'blank/whitespace-only codes are ignored (treated as no restriction)',
      () {
        expect(LocalizationConfig.resolve(bundled, const ['   ', '']), bundled);
      },
    );
