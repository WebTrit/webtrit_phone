...
void main() {
  ...
  group('ThumbnailFrame', () {
    ...
    testWidgets('a screen reader gets one named button for the window', (tester) async {
      ...
      final window = find.bySemanticsIdentifier('thumbnail');
      expectTapTargetSemantics(tester, window, label: 'Return to the call', identifier: 'thumbnail', isButton: true);
      await tapViaSemantics(tester, window);
      expect(taps, 1);
      semantics.dispose();
    });

    testWidgets('an inert window announces nothing at all', (tester) async {
      final semantics = tester.ensureSemantics();
      await tester.pumpWidget(
        wrap(
          const ThumbnailFrame(
      ...
      // Naming a control that cannot be activated is worse than staying quiet:
      // it invites a press that does nothing.
      expect(find.bySemanticsIdentifier('preview'), findsNothing);
      semantics.dispose();
    });
  });
}
