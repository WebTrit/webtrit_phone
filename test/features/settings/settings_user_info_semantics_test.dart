...
void main() {
  ...
  testWidgets('the refresh button of the account screen says what it refreshes', (tester) async {
    final handle = tester.ensureSemantics();

    await tester.pumpWidget(wrapScreen());

    // The only control in the app bar of the account screen, and a bare circular
    // arrow with nothing beside it to say what it brings up to date.
    expectTapTargetSemantics(
      tester,
      find.bySemanticsIdentifier(settingsRefreshId),
  ...
  testWidgets('pressing refresh through semantics asks the server for the status again', (tester) async {
    final handle = tester.ensureSemantics();

    await tester.pumpWidget(wrapScreen());

    await tapViaSemantics(tester, find.bySemanticsIdentifier(settingsRefreshId));
